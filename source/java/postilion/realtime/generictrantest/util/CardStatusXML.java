...
public class CardStatusXML extends Iso8583 {

	public Iso8583 resultCardStatusXML(Iso8583 p_msg, Iso8583Post msgToTM, boolean enableLog, String process) {
		...
		try {
			...
			// Se invoca el procedimiento almacenado para obtener los datos del cliente
			...
			// Se procesa el estado de la tarjeta
			resultCardStatus = processCardStatusXML(p_msg, msgToTM, field_structured, enableLog);

			// Se verifica si el campo 102 esta configurado
			if (p_msg.isFieldSet(Iso8583.Bit._102_ACCOUNT_ID_1)) {
				...
				if (SystemConstants.TWO_ZEROS.equals(primerasDosPosiciones)) {
					...
					p_msg.putField(Iso8583Post.Bit._059_ECHO_DATA, XmlHandler.buildSuccessfulResponse());
					field_structured.put(SystemConstants.KEY_NOV_CAPA, SystemConstants.VALUE_NOV_CAPA);
					// Limpieza del ID del cliente
					...
					p_msg.putField(Iso8583Post.Bit._059_ECHO_DATA, XmlHandler.buildFailedResponse());
				}
			}
	...
	public boolean processCardStatusXML(Iso8583 p_msgIso, Iso8583Post msgToTM, StructuredData field_structured,
			boolean enableLog) {
		try {

			String xmlInput = p_msgIso.getField(Iso8583Post.Bit._059_ECHO_DATA);
			String pan = XmlHandler.processXmlRequest(xmlInput, SystemConstants.CARD_NUM, enableLog);
			String cardStatus = XmlHandler.processXmlRequest(xmlInput, SystemConstants.CARD_STATUS, enableLog);

			// Obtencion de valores de structured data directamente
